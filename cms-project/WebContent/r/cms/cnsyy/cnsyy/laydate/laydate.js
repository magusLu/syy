...
!function () {
	...
	T.prototype.render = function () {
		...
		if (t.zIndex && (i.style.zIndex = t.zIndex), w.each(new Array(2), function (e) {
				...
				m = w.elem("thead"),
				u = w.elem("tr");
				c.setAttribute("class","table_schedule_c");
				...
				w.each(new Array(6), function (e) {
					...
					w.each(new Array(num), function (a) {
						if (0 === e) {
							var i = w.elem("th");
	...
	T.prototype.calendar = function (e) {
		...
		var day3;
		if(s.moreFlag){
			day1 = new Date(s.startDate.replace(new RegExp(/-/gm) ,"/"));
			day3 = new Date(s.endDate.replace(new RegExp(/-/gm) ,"/"));
		}
		if (l.year < d[0] && (l.year = d[0], r.hint("最低只能支持到公元" + d[0] + "年")), l.year > d[1] && (l.year = d[1], r.hint("最高只能支持到公元" + d[1] + "年")), r.firstDate || (r.firstDate = w.extend({}, l)), c.setFullYear(l.year, l.month, 1), t = c.getDay(), a = n.getEndDate(l.month || 12, l.year), i = n.getEndDate(l.month + 1, l.year), w.each(y, function (e, n) {
				...
				n = w(n),
				n.removeAttr("class"),
				e < t ? (c = a - t + e, n.addClass("laydate-day-prev"), d = r.getAsYM(l.year, l.month, "sub")) : e +mf>= t && e+mf < i + t  ? (c = e - t + mf, s.range || c + 1 === l.date && n.addClass(o)) : (c = e - i - t+mf, n.addClass("laydate-day-next"), d = r.getAsYM(l.year, l.month)),
				d[1]++;
				d[2] = c + 1;
				if((e%8<7&&s.moreFlag)||!s.moreFlag){
					...
					if(day1<day2&&day2<day3){
						...
						var dd=Math.ceil(days/7);
						
						n.attr("class", "td_teach_c").html('<span class="numb">'+dd+'</span>');
					}else{
						n.attr("lay-ymd", d.join("-")).html("");
					}
				}
				r.mark(n, d).limit(n, {
					year : d[0],
